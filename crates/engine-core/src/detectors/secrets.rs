...
static GITHUB_TOKEN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"gh[pousr]_[A-Za-z0-9]{36,255}|github_pat_[A-Za-z0-9_]{82}").unwrap());

static PEM_BLOCK: Lazy<Regex> = Lazy::new(|| Regex::new(r"-----BEGIN [A-Z ]{1,40}-----").unwrap());
